...
def test_get_value():
    ...
    print("Test 6 passed: 'num/value' == 123")

    assert get_value({"flag": {"enabled": True}}, "flag/enabled") is True
    print("Test 7 passed: 'flag/enabled' == True")

    print("\nRunning test: error handling")

    try:
        get_value({"a": {"b": {}}}, "a/b/c")
        print("Test 8 failed: Missing key did not raise KeyError")
    except KeyError:
        print("Test 8 passed: Caught missing key as KeyError")

    try:
        get_value({"a": 1}, "a/b")
        print("Test 9 failed: Wrong type did not raise TypeError")
    except TypeError:
        print("Test 9 passed: Caught wrong type as TypeError")
